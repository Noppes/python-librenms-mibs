# SNMP MIB module (CUMULUS-BGPUN-MIB) expressed in pysnmp data model.
#
...
# Notes
# -----
# ASN.1 source file://mibs\cumulus\CUMULUS-BGPUN-MIB
...
class _BgpPeerIface_Type(OctetString):
    """Custom type bgpPeerIface based on OctetString"""
    subtypeSpec = OctetString.subtypeSpec
    subtypeSpec += ConstraintsUnion(
...
_BgpPeerIface_Type.__name__ = "OctetString"
...
class _BgpPeerDesc_Type(OctetString):
    """Custom type bgpPeerDesc based on OctetString"""
    subtypeSpec = OctetString.subtypeSpec
    subtypeSpec += ConstraintsUnion(
        ValueSizeConstraint(2, 255),
    )
...
_BgpPeerDesc_Type.__name__ = "OctetString"
_BgpPeerDesc_Object = MibTableColumn
bgpPeerDesc = _BgpPeerDesc_Object(
    (1, 3, 6, 1, 4, 1, 40310, 4, 3, 1, 1, 26),
    _BgpPeerDesc_Type()
)
...
bgp4PathAttrTable = _Bgp4PathAttrTable_Object(
    (1, 3, 6, 1, 4, 1, 40310, 4, 5)
)
if mibBuilder.loadTexts:
    bgp4PathAttrTable.setStatus("current")
_Bgp4PathAttrEntry_Object = MibTableRow
bgp4PathAttrEntry = _Bgp4PathAttrEntry_Object(
    (1, 3, 6, 1, 4, 1, 40310, 4, 5, 1)
)
bgp4PathAttrEntry.setIndexNames(
    (0, "CUMULUS-BGPUN-MIB", "bgp4PathAttrIpAddrPrefix"),
    (0, "CUMULUS-BGPUN-MIB", "bgp4PathAttrIpAddrPrefixLen"),
    (0, "CUMULUS-BGPUN-MIB", "bgp4PathAttrPeer"),
)
if mibBuilder.loadTexts:
...
bgpEstablished = NotificationType(
    (1, 3, 6, 1, 4, 1, 40310, 4, 7, 1)
)
bgpEstablished.setObjects(
      *(("CUMULUS-BGPUN-MIB", "bgpPeerLastError"),
        ("CUMULUS-BGPUN-MIB", "bgpPeerState"))
)
if mibBuilder.loadTexts:
