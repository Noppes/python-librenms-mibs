# SNMP MIB module (PROSTAR-PWM) expressed in pysnmp data model.
#
...
# Notes
# -----
# ASN.1 source file://mibs\morningstar\PROSTAR-PWM
...
prostarPwm = ModuleIdentity(
    (1, 3, 6, 1, 4, 1, 33333, 6)
)
if mibBuilder.loadTexts:
    prostarPwm.setRevisions(
        ("2019-12-20 00:00",)
    )
...
class AlarmsBitfield(TextualConvention, Bits):
    ...
    namedValues = NamedValues(
        *(("rtsOpen", 0),
          ...
          ("heatsinkTempLimit", 5),
          ("currentLimit", 6),
          ("currentMeasurementError", 7),
          ("batterySenseOutOfRange", 8),
          ("batterySenseDisconnected", 9),
          ("uncalibrated", 10),
          ("batteryTempOutOfRange", 11),
          ("fp10SupplyOutOfRange", 12),
          ("mosfetOpen", 13),
          ("arrayCurrentOffset", 14),
          ("loadCurrentOffset", 15),
          ("p33SupplyOutOfRange", 16),
          ("p12SupplyOutOfRange", 17),
          ("hightInputVoltageLimit", 18),
          ("controllerReset", 19),
          ("loadLvd", 20),
          ("logTimeout", 21),
          ("eepromAccessFailure", 22))
    )
...
class _Fp10Voltage_Type(DisplayString):
    """Custom type fp10Voltage based on DisplayString"""
    subtypeSpec = DisplayString.subtypeSpec
    subtypeSpec += ConstraintsUnion(
        ValueSizeConstraint(0, 16),
    )
...
_Fp10Voltage_Type.__name__ = "DisplayString"
_Fp10Voltage_Object = MibScalar
fp10Voltage = _Fp10Voltage_Object(
    (1, 3, 6, 1, 4, 1, 33333, 6, 42),
    _Fp10Voltage_Type()
)
fp10Voltage.setMaxAccess("read-only")
if mibBuilder.loadTexts:
    fp10Voltage.setStatus("current")


class _P3Supply_Type(DisplayString):
    """Custom type p3Supply based on DisplayString"""
    subtypeSpec = DisplayString.subtypeSpec
    subtypeSpec += ConstraintsUnion(
        ValueSizeConstraint(0, 16),
    )
...
_P3Supply_Type.__name__ = "DisplayString"
_P3Supply_Object = MibScalar
p3Supply = _P3Supply_Object(
    (1, 3, 6, 1, 4, 1, 33333, 6, 43),
    _P3Supply_Type()
)
p3Supply.setMaxAccess("read-only")
if mibBuilder.loadTexts:
    p3Supply.setStatus("current")


class _MeterVoltage_Type(DisplayString):
    """Custom type meterVoltage based on DisplayString"""
    subtypeSpec = DisplayString.subtypeSpec
    subtypeSpec += ConstraintsUnion(
        ValueSizeConstraint(0, 16),
    )
...
_MeterVoltage_Type.__name__ = "DisplayString"
_MeterVoltage_Object = MibScalar
meterVoltage = _MeterVoltage_Object(
    (1, 3, 6, 1, 4, 1, 33333, 6, 44),
    _MeterVoltage_Type()
)
meterVoltage.setMaxAccess("read-only")
if mibBuilder.loadTexts:
    meterVoltage.setStatus("current")
_ChargeState_Type = ChargeStateEnum
_ChargeState_Object = MibScalar
chargeState = _ChargeState_Object(
    (1, 3, 6, 1, 4, 1, 33333, 6, 45),
    _ChargeState_Type()
)
